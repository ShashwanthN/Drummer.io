...
const TopBar = () => {
    ...
    return (
        <div className={`topbar fixed top-0 left-0 z-10 w-full  flex divide-secondary divide-y divide-y-reverse bg-ascent transition-all duration-1000 ${isScrolled ? 'shadow-lg' : ''}`}>
            ...
            <div className='w-full font-Roboto flex items-center text-bg2 justify-between py-2 md:py-1 px-4 bg-bg'>
                ...
                <div className='flex gap-4 items-center justify-center text-secondary font-regular hover:text-shadow-3xl text-md md:text-xl relative'>
                    ...
                    <button className='text-ascent  hover:text-text border-secondary md:pr-2 ' onClick={() => handleTheme()}>
                        {theme === 'light' ?<div className='flex items-center '> <MdDarkMode className='md:mr-2' size={25} /> <span className='hidden md:block font-bold'>Dark</span></div> : <div className='flex items-center '> <MdLightMode className='md:mr-2' size={25} /> <span className='hidden font-bold md:block'>Light</span></div>}
                    </button>
